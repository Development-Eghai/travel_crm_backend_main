...
def create_blog_category(cat_in: BlogCategoryCreate, db: Session = Depends(get_db)):
    try:
        ...
        db.commit()
        db.refresh(category)
        return api_json_response_format(True, "Category created successfully.", 201, BlogCategoryOut.model_validate(category).model_dump())
...
def list_blog_categories(db: Session = Depends(get_db)):
    try:
        categories = db.query(BlogCategory).filter(BlogCategory.is_active == True).order_by(BlogCategory.sort_order).all()
        data = [BlogCategoryOut.model_validate(c).model_dump() for c in categories]
...
def get_blog_category(category_id: int, db: Session = Depends(get_db)):
    try:
        ...
        return api_json_response_format(True, "Category retrieved successfully.", 200, BlogCategoryOut.model_validate(category).model_dump())
    except Exception as e:
...
def update_blog_category(category_id: int, cat_in: BlogCategoryCreate, db: Session = Depends(get_db)):
    try:
        ...
        db.commit()
        db.refresh(category)
        return api_json_response_format(True, "Category updated successfully.", 200, BlogCategoryOut.model_validate(category).model_dump())
    except Exception as e:
